...
for (netw in c('clics3','omegawiki','freedict'))
{
  for (lex in c('WKB','NRCVAD'))
  {
    ...
    diff<-1-(var2/var1)
    df<-data.frame(network=netw,lexicon=lex,var=round(diff*100,digits = 1),stringsAsFactors = F)
...
Figure 8. Comparison with the null models in the case of OmegaWiki and the NRC VAD lexicon
```{r}
netw<-'omegawiki'  ##selection of the network and the lexicon
lex<-'NRCVAD'
iter_perm<-10000 ##number of iterations of the permutation
